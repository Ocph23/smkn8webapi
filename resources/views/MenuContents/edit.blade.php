...
<div class="grid grid-cols-1 gap-9 sm:grid-cols-2">
  <div class="flex flex-col gap-9">
    ...
    <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
      ...
      <form action="/admin/menu/put" id="add-blog-post-form" method="post">
        @csrf
        <div class="p-6.5">
          <div class="mb-4.5">
            ...
            <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent dark:bg-form-input">
              <select name="parent_id" class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent px-5 py-3 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary" :class="isOptionSelected &amp;&amp; 'text-black dark:text-white'" @change="isOptionSelected = true">
                ...
                @else
                <option value="{{$data->parent_id}}" class="text-body">{{$data->parent->menu }}</option>
                @endif
              </select>
          ...
          <div class="mb-4.5">
            ...
            <input type="text" name="menu" placeholder="nama menu" value="{{$data->menu}}" class="w-full rounded border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary">
          </div>
         
          <input name="level" value="{{$data->level}}" hidden>
